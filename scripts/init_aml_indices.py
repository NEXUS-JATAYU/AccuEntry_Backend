...
def init_aml_indices():
    ...
    try:
        ...
        print("[OK] Created text index on ofac_sdn_list (name, aliases)")
    except Exception as e:
        print(f"[WARN] ofac_sdn_list text index: {e}")
    
    try:
        aml_db.ofac_sdn_list.create_index([("active", 1), ("uid", 1)], name="ofac_active_uid")
        print("[OK] Created compound index on ofac_sdn_list (active, uid)")
    except Exception as e:
        print(f"[WARN] ofac_sdn_list compound index: {e}")
    
    # PEP List indices
    try:
        aml_db.pep_list.create_index([("name", "text")], name="pep_text_search")
        print("[OK] Created text index on pep_list (name)")
    except Exception as e:
        print(f"[WARN] pep_list text index: {e}")
    
    try:
        aml_db.pep_list.create_index([("active", 1), ("pep_tier", 1)], name="pep_active_tier")
        print("[OK] Created compound index on pep_list (active, pep_tier)")
    except Exception as e:
        print(f"[WARN] pep_list compound index: {e}")
    
    # RBI Caution List indices
    try:
        aml_db.rbi_caution_list.create_index([("pan", 1), ("active", 1)], name="rbi_pan_active")
        print("[OK] Created compound index on rbi_caution_list (pan, active)")
    except Exception as e:
        print(f"[WARN] rbi_caution_list index: {e}")
    
    # Risk Rules indices
    try:
        aml_db.risk_rules.create_index([("active", 1)], name="risk_rules_active")
        print("[OK] Created index on risk_rules (active)")
    except Exception as e:
        print(f"[WARN] risk_rules index: {e}")
    
    print("\n[OK] AML indices initialization complete!")
